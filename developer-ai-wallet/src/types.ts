...
export interface X402DemoConfigSnapshot {
  name: string;
  mode: 'simulation';
  type: 'x402_payment';
  owner_wallet: string;
...
export interface X402PaymentCheckInput {
  demoApiKey?: string;
  scenario?: 'allowed' | 'blocked' | 'custom';
  intent: string;
  action?: string;
  domain: string;
  endpoint: string;
...
export interface X402PaymentCheckResult extends DemoIntentResult {
  projectId: string;
  payment: {
    intent: string;
    action: string;
